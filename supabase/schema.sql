-- =============================================================================
-- Moderna (MRNA) Airdrop Engine — Supabase schema
...
create table if not exists public.cycles (
  ...
  reward_bought_raw  numeric(40,0) not null default 0,

  -- distribution leg
  reward_distributed_raw numeric(40,0) not null default 0,
...
create table if not exists public.snapshot_holders (
  ...
  allocation_raw numeric(40,0) not null default 0,
...
-- ---------------------------------------------------------------------------
-- payouts: one row per (cycle, wallet). The unique key is the idempotency key
-- that makes a crashed / restarted worker safe to resume.
-- ---------------------------------------------------------------------------
...
select
  ...
  (select coalesce(sum(reward_bought_raw), 0) from public.cycles)            as total_reward_bought_raw,
  (select coalesce(sum(amount_raw), 0) from public.payouts
     where status = 'confirmed')                                            as total_reward_distributed_raw,
